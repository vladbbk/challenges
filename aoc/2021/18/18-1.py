...
snails = [ Snailfish ( line ) for line in lines ]
cartesian = product ( range ( len ( snails) ), range ( len ( snails ) ) )


for c in cartesian:
    first_snail, second_snail = deepcopy (snails [ c[0] ]), deepcopy(snails [ c[1] ])
    snail = first_snail + second_snail

    magnitude = snail.compute_maginutude ()
    if magnitude > max_magnitude:
        max_magnitude = magnitude
        winners = (first_snail, second_snail )



print ( f"winners: {winners[0]}\n{winners[1]}")
